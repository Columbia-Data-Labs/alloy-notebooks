...
function buildRWrapper(rCode: string): string {
  ...
  return [
    ...
    '',
    '# Auto-detect R_HOME across Windows, Linux, and macOS',
    'if not _os.environ.get("R_HOME"):',
    '    import platform as _plat',
    '    _candidates = []',
    '    if _plat.system() == "Windows":',
    '        import glob as _glob',
    '        _candidates = sorted(_glob.glob("C:/Program Files/R/R-*"), reverse=True)',
    '    elif _plat.system() == "Darwin":',
    '        import glob as _glob',
    '        _candidates = sorted(_glob.glob("/Library/Frameworks/R.framework/Versions/*/Resources"), reverse=True)',
    '        _candidates += ["/usr/local/lib/R", "/opt/homebrew/lib/R", "/opt/R/current"]',
    '    else:',
    '        _candidates = ["/usr/lib/R", "/usr/local/lib/R", "/usr/lib64/R", "/opt/R/current"]',
    '        import glob as _glob',
    '        _candidates += sorted(_glob.glob("/opt/R/*/lib/R"), reverse=True)',
    '    for _rdir in _candidates:',
    ...
    '    except: pass',
    '',
    '# Add R shared libraries to PATH/LD path (Windows needs DLL dir)',
    ...
    '        except: pass',
    '    else:',
    '        _rlib = _os.path.join(_os.environ["R_HOME"], "lib")',
    '        if _rlib not in _os.environ.get("LD_LIBRARY_PATH", ""):',
    '            _os.environ["LD_LIBRARY_PATH"] = _rlib + ":" + _os.environ.get("LD_LIBRARY_PATH", "")',
    '    try: del _plat',
    '    except: pass',
    '',
    'try:',
